...
echo th neural_style.lua -image_size "$image_size" -num_iterations "$num_iteractions" -style_image "$pattern_path" -content_image "$image_path" -backend cudnn -output_image "$out_path"
